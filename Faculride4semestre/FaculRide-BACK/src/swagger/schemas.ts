export const swaggerDocument = {
  ...
  paths: {
    ...
    '/usuario/{id}': {
     ...
     // Usuário - Exportar para Excel (alinhado a IusuarioFiltros)
'/usuario/exportar/excel': {
  get: {
    summary: 'Exportar usuários para Excel',
    description: 'Gera e retorna um arquivo Excel (.xlsx) com a lista de usuários.',
    tags: ['Usuário'],
    security: [{ bearerAuth: [] }],
    parameters: [
      {
      ...
      },
      {
        ...
        example: 'maria@faculride.com'
      },
      {
        name: 'tipoUsuario',
        in: 'query',
        schema: { type: 'string', enum: ['passageiro', 'motorista'] },
        description: 'Filtra por tipo de usuário',
        example: 'motorista'
      },
      // Paginação (opcional, mantenha apenas se a rota suporta)
      {
        name: 'page',
        in: 'query',
      ...
      },
      {
        ...
        description: 'Quantidade por página (opcional)'
      }
    ],
    responses: {
      200: {
        ...
        headers: {
          'Content-Disposition': {
            schema: { type: 'string', example: 'attachment; filename="usuarios.xlsx"' },
            description: 'Sugestão de nome do arquivo para download'
          }
        },
        content: {
      ...
      500: { description: 'Erro ao gerar o arquivo' }
    }
  }
},
